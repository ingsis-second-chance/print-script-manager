...
public class Utils {
  ...
  public static <T> T postRequest(
      RestTemplate webClient, String path, HttpEntity<?> request, Class<T> responseType) {
  ...
  public static <T> T getRequest(
    ...
    Map<String, List<String>> multiValueParams =
        params.entrySet().stream()
            .collect(
                Collectors.toMap(
                    Map.Entry::getKey, e -> Stream.of(e.getValue()).collect(Collectors.toList())));

    String urlTemplate =
        UriComponentsBuilder.fromHttpUrl(createUrl(webClient, path))
            .queryParams(CollectionUtils.toMultiValueMap(multiValueParams))
            .toUriString();

    ResponseEntity<T> response =
        webClient.exchange(urlTemplate, HttpMethod.GET, request, responseType, params);
